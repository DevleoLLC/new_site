...
define('THEME_BUYNOW_URL', 'https://wrapbootstrap.com/theme/appstrap-responsive-website-template-WB0C6D0H4?ref=tme');
define('THEME_UPSELL_URL', 'https://wrapbootstrap.com/theme/appstrap-responsive-website-template-WB0C6D0H4?ref=tme');
...
elseif ($_GET['tpl'] == 'screenshots') {
  foreach ($all_pages as $k => $data) {
    ...
    if (file_exists($php_tpl) && !empty($data['make_screenshot'])) {
      ...
      generate_thumbnail_from_url('http://local.appstraptheme.com/render.php?tpl='. $k .'&screenshot=true', $k .'.png', $crop_options);
    }
  }
  print "--------------------------------------<br /><br />";
...
//specific template
  ...
  if ($_GET['make_screenshot']) {
    ...
    generate_thumbnail_from_url('http://local.appstraptheme.com/render.php?tpl='. $_GET['tpl'] .'&screenshot=true', $_GET['tpl'] .'.png', $crop_options);
